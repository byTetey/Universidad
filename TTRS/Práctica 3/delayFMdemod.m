...
fm= 200; 		% Frec moduladora (Hz);, modifica que tan separadas estas las componentes en la representaciÃ³n de la DEP
...
j=sqrt(-1);
m=sin(2*pi*fm*t); % Modulating signal, seÃ±al moduladora
...
xbb=xfm.*exp(-j*2*pi*(fc+difff)*t); % la señal xfm en banda base
...
% Es la parte anticausal de la señal, lo que está antes del cero, que tiene
% una longitud L/2. A partir de aquí es influyente esta parte de la señal.
...
% calulamos el producto de la señal retardada y conjugada de la señal
w=xbb.*conj(y1);
